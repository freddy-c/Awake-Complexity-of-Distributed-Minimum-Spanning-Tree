...
def fragment_broadcast(self, start_round: int, message: str = None):
    ...
    if self.root:
        schedule_items = [
            (1 + base, Procedure.FRAGMENT_BROADCAST, TransmissionRound.DOWN_SEND), # OPTIMIZATION
            (
                self.maximum_depth + 2 + base, # OPTIMIZATION
        ...
        schedule_items = [
            (
                self.i + base, # OPTIMIZATION
                Procedure.FRAGMENT_BROADCAST,
                TransmissionRound.DOWN_RECEIVE,
            ),
            (
                self.i + 1 + base, # OPTIMIZATION
                Procedure.FRAGMENT_BROADCAST,
